SELECT
	users.id AS user_id,
	COALESCE(winners.total_book_count, 0) AS score,
	...
	CASE
		...
		WHEN COALESCE(losers.losses, 0) = 0 THEN 100
...
LEFT JOIN (
	SELECT
		...
		SUM(winners.books) AS total_book_count
	FROM users
...
ORDER BY score DESC, win_rate DESC, highest_book_count DESC