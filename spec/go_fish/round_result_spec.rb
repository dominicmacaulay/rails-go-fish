...
RSpec.describe RoundResult do
  ...
  context '#==' do
    it 'returns true if the stored strings are the same' do
      result1 = RoundResult.new(id: 1, player: player1, opponent: player2, rank: '2', fished: false, got_rank: true,
                                card_gotten: '2', amount: 'two', empty_pond: true)
      result2 = RoundResult.new(id: 1, player: player1, opponent: player2, rank: '2', fished: false, got_rank: true,
                                card_gotten: '2', amount: 'two', empty_pond: true)
      expect(result1).to eq result2
    end
    it 'returns false if the stored string are not the same' do
      result1 = RoundResult.new(id: 1, player: player1, opponent: player2, rank: '2')
      result2 = RoundResult.new(id: 1, player: player3, opponent: player2, rank: '2')
  ...
  context '#display' do
    context 'for the current_player' do
      it 'gets rank from opponent' do
        result = RoundResult.new(id: 1, player: player1, opponent: player2, rank: '2', got_rank: true, amount: 'two')
      ...
      it 'gets rank from pond' do
        result = RoundResult.new(id: 1, player: player1, opponent: player2, rank: '2', fished: true, got_rank: true)
        result_message = result.generate_message_for(player1)
        expect(result_message.action).to eql "You asked P2 for 2's"
      ...
      it 'does not get rank from the pond' do
        result = RoundResult.new(id: 1, player: player1, opponent: player2, rank: '2', fished: true, card_gotten: '4')
        result_message = result.generate_message_for(player1)
        expect(result_message.action).to eql "You asked P2 for 2's"
        expect(result_message.opponent_response).to eql "Go Fish! P2 did not have any 2's"
